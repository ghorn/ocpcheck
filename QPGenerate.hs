...
module QPGenerate
( ParamOptQP, ParamOptL, ParamCst, BoundsSpace, BoundsCst
, generateProblem
, generateProblemSF
, mult
, epsilon
...
type ParamOptQP = [Double]
type ParamOptL = [Double]
type ParamCst = [Double]
type BoundsSpace = [(Double,Double)]
type BoundsCst = [(Double,Double)]

generateProblem :: Integer -> Bool -> Int -> Int -> ([Double], ParamOptQP, ParamOptL, ParamCst, BoundsSpace, BoundsCst)
...
generateProblemSF :: Integer -> Int -> Int -> Integer -> Integer -> ([Double], ParamOptQP, ParamOptL, ParamCst, BoundsSpace, BoundsCst)
generateProblemSF gen n m minB maxB = (x0, optqp, opt, pcst, bsp, bcst)
  where
  -- Generate parameters of the objective function
    ...
    gen4':moptqp = take (n*n+1) $ randomRs (minB, maxB) (mkStdGen (fromInteger gen1 :: Int)) :: [Integer]  
    gen4 = if gen4' == gen1 then (gen4'+1) else gen4'
    optqp = zipWith (*) 
                  (map fromInteger moptqp) 
                  (take (n*n) $ randoms (mkStdGen (fromInteger gen1 :: Int)) :: [Double])
  -- Generate parameters of the matrix A
    gen2':mpcst = take (n*m+1) $ randomRs (minB, maxB) (mkStdGen (fromInteger gen4 :: Int)) :: [Integer]  
    gen2 = if gen2' == gen1 then (gen2'+1) else gen2'
    pcst = zipWith (*) 
                  (map fromInteger mpcst)
                  (take (m*n) $ randoms (mkStdGen (fromInteger gen4 :: Int)) :: [Double])
...
generateProblemU :: Integer -> Int -> Int -> Integer -> Integer -> ([Double], ParamOptQP, ParamOptL, ParamCst, BoundsSpace, BoundsCst)
generateProblemU gen n m minB maxB = (x0, optqp, opt, pcst, bsp, bcst)
  where
  -- Generate parameters of the objective function
    gen':mopt = take (n+1) $ randomRs (minB, maxB) (mkStdGen (fromInteger gen :: Int)) :: [Integer]  
    gen1 = if gen' == gen then (gen'+1) else gen'
    opt = zipWith (*) 
                  (map fromInteger mopt) 
                  (take n $ randoms (mkStdGen (fromInteger gen :: Int)) :: [Double])
    gen6':moptqp = take (n*n+1) $ randomRs (minB, maxB) (mkStdGen (fromInteger gen1 :: Int)) :: [Integer]  
    gen8 = if gen6' == gen1 then (gen6'+1) else gen6'
    optqp = zipWith (*) 
                  (map fromInteger moptqp) 
                  (take (n*n) $ randoms (mkStdGen (fromInteger gen1 :: Int)) :: [Double])
  -- Generate parameters of the matrix A
    gen2:mpcst = take (n*m+1) $ randomRs (minB, maxB) (mkStdGen (fromInteger gen8 :: Int)) :: [Integer]  
    pcst = zipWith (*) 
                  (map fromInteger mpcst)
                  (take (m*n) $ randoms (mkStdGen (fromInteger gen8 :: Int)) :: [Double])
