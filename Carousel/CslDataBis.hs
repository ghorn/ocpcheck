...
module CslDataBis ( CX(..) , U(..) , Cd(..) , diff , modelForces , Cdata (..), modelCsl ) where
...
data Cd a = Cd { drag :: a } deriving (Eq, Functor, Generic, Generic1, Show)
...
modelForces ::forall a . (Floating a) => CX a -> CX a -> U a -> Cd a -> CX a
modelForces (CX d' alp' alpd' b' bd') (CX d alp alpd b bd) (U u) (Cd cw) =
  ...
  where
    ...
    Cdata pA roh _ r0 l m = modelCsl

    resA = m*r0*(-alpd') - m*r0*cos theta *sin theta * pd**2 + sin theta *( m*g+f3) - sin phi * cos theta * f2 - m*l*sin phi *cos theta * d**2 - m*cos theta*d**2 *r0*sin theta +2* cos theta *sin theta * m*r0*d*pd - cos phi * cos theta * (f1+m*u*l)
